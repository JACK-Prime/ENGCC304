...
void most_em_salary(int , EM * , int * ) ; //the most employee's salary
...
int main () {
    ...
    // index ที่เก็บตำแหน่งเงินเดือนสูงสุด
    most_em_salary(i, salary, &H_Index ) ; // --> H_Index มันคือ address 
...
void most_em_salary(int count, EM *salary ,int *H_Index) { //find the most employee's salary
